...
function getContentTimestamp(frontmatter: ContentFrontmatter): number {
  if (frontmatter.lastModified) {
    return new Date(frontmatter.lastModified).getTime();
  }
  if (frontmatter.date) {
...
export async function getAllContent(
  ...
  return items.sort((a, b) => {
    ...
    if (!a.frontmatter || !b.frontmatter) {
      console.warn("Missing frontmatter in content item:", {
        a: a.slug,
        b: b.slug,
      });
      return 0;
    ...
    const timeDiff =
      getContentTimestamp(b.frontmatter) - getContentTimestamp(a.frontmatter);
...
export function isValidLanguage(lang: string): lang is Language {
  const validLanguages: Language[] = ["en", "th", "ja", "ko"];
