...
LANGUAGES_FILE = PROJECT_ROOT.parent / '0_meta' / PROJECT_ROOT.name / 'languages.json'
...
def load_config() -> dict[str, Any]:
    ...
    api_key = (
        os.getenv(config.get('api_key_env', 'OPENAI_API_KEY'))
        or config.get('api_key')
        or ''
    ).strip()
    if not api_key:
        print('[FAIL] Missing translation API key in environment/config')
        sys.exit(1)

    config['api_key'] = api_key
    api_base_url = os.getenv(config.get('api_base_url_env', '')) or config.get('api_base_url', '')
    model = os.getenv(config.get('model_env', '')) or config.get('model', '')
    if api_base_url:
...
def load_target_languages(config: dict[str, Any]) -> list[str]:
    if LANGUAGES_FILE.exists():
      with open(LANGUAGES_FILE, 'r', encoding='utf-8') as handle:
          data = json.load(handle)
      langs = [
          str(code).strip()
          for code in data.get('translation_codes', [])
...
def extract_translated_items(parsed: Any) -> dict[str, str]:
...
def translate_chunk(
    ...
    try:
        ...
        translated_by_id = extract_translated_items(parsed)
...
def main() -> None:
    ...
    if args.lang:
        ...
        languages = load_target_languages(config)
